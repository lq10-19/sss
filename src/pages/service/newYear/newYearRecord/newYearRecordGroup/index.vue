<template>
  <div class="main">
    ...
    <div class="content" :style="{'min-height': windowHeight+'rpx'}">
      <!-- 邀请好友 -->
      <div class="topTitle"><p>组团给 <span>{{toNickname}}</span> 拜年</p></div>
      <div v-if="status==2" class="topTitle_sub"><p>祝福已经发送，待对方回应</p></div>
      <div class="friend">
        <div class="friendList">
          <ul>
            <li v-show="item.greetingsValue" v-for="(item,index) in groupLsit" :key="index" :class="{'changeClass':item.sendType=='1'}" class="clearfix">
              <div class="friendName">
                <img :src="item.fromAvatarUrl" alt="">
                <p>{{item.fromNickname}}</p>
              </div>
              <div class="msg alreadyRead">
                <p v-if="item.greetingsValue">{{item.greetingsValue}}</p>
              </div>
            </li>
        ...
        <!-- 发起拜年 -->
      <div @click="goGroupList" class="myButton">
        <p>给其他人拜年</p>
      </div>
      <!-- 返回主页 -->
...
export default {
  ...
  methods: {
    getRecordDetail(){
      ...
      let url = path.recordDetail+"/"+this.groupMainId;
