<template>
  <div class="main">
    ...
    <div class="content" :style="{'min-height': windowHeight+'rpx'}">
      ...
      <div class="topTitle"><p>小伙伴们组团给您拜年啦！</p></div>
      <!-- 祝福list表 -->
      <div class="friend">
        <div class="friendList">
          <ul>
            <li v-show="item.greetingsValue" v-for="(item,index) in groupLsit" :key="index" :class="{'changeClass':item.sendType=='1'||item.sendType=='2'}" class="clearfix">
      ...
      <div v-if="recordListIsShow" class="moneyNumber"><p>已打赏红包总额{{moneyNumber}}元</p></div>
      <!-- 红包记录表 -->
      <div v-if="recordListIsShow" class="recordList">
        <ul>
          <li v-for="(item,index) in redBagList" :key="index" class="clearfix">
            <div class="imgClass">
              <div><img :src="item.toAvatarUrl" alt=""></div>
              <div>
                <p>{{item.toNickname}}</p>
                <p>{{item.createTime}}</p>
              </div>
            </div>
            <div class="recordMoneyNumber"><p>{{item.redPackage}}元</p></div>
          </li>
...
export default {
  ...
  onLoad(options) {
    this.moneyNumber = 0;
    if(options.status=="6"){
      this.recordListIsShow = true;
    }else{
      this.recordListIsShow = false;
    }
    this.groupLsit=[];
      ...
      promise
        .then(data => {
          if (data != null&&data.length>0) {
            ...
            this.redBagList = sendType2List;
            this.redBagList.forEach((v,i)=>{
              this.moneyNumber+=Number(v.redPackage);
            })
          } else {
            this.isShow= false;
          }
